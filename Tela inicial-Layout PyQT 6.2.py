...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        primeira_layout.addWidget(container_esquerdo, 4)  # Aumentada a proporção para 4
        primeira_layout.addWidget(self.create_buttons_area(), 1)  # Mantém proporção 1, mas com largura máxima definida
        ...
        button_style = """
            QPushButton {
                background-color: #3498db;  /* Azul similar aos botões do Chat */
                color: white;
                border: none;
                border-radius: 4px;
                padding: 8px 16px;
            ...
            QPushButton:hover {
                background-color: #2980b9;  /* Azul mais escuro para hover */
            }
            QPushButton:pressed {
                background-color: #2472a4;  /* Azul ainda mais escuro quando pressionado */
            }
        """
        ...
        # Configurar visualizador web com tema escuro (método alternativo)
        web_profile = QWebEngineProfile.defaultProfile()
        web_settings = web_profile.settings()
        
        # Aplicar configurações dark para todos os QWebEngineView
        for web_view in self.findChildren(QWebEngineView):
            page = web_view.page()
            page.setBackgroundColor(QColor(47, 53, 59))  # Cinza chumbo
            
            # Injetar CSS para forçar tema escuro
            dark_mode_js = """
            (function() {
            ...
            """
            page.runJavaScript(dark_mode_js)
    ...
    def set_active_button(self, button, frases):
        ...
        if self.active_button is not None:
            try:
                self.active_button.setStyleSheet(BUTTON_STYLE)
            except RuntimeError:
                self.active_button = None

        # Define o novo botão ativo
        self.active_button = button
        self.active_button.setStyleSheet(ACTIVE_BUTTON_STYLE)
    ...
    def create_buttons_area(self):
        ...
        # Definir o estilo base dos botões como uma constante
        BUTTON_STYLE = """
            QPushButton {
                text-align: center;
                font-family: Arial;
        ...
        for segmento, botoes in self.segmentos.items():
            ...
            section_layout.setContentsMargins(2, 0, 2, 0)
        ...
        # Estilo simplificado para a área de scroll
    ...
    def create_filter_layout(self):
        ...
        open_folder_button = QPushButton("Lista")
        open_folder_button.setFixedSize(50, 30)
        open_folder_button.clicked.connect(self.show_most_used_phrases)
    ...
    def open_outlook_email(self, template_type):
        """
        Busca o último e-mail enviado com o termo específico na pasta Itens Enviados
        """
        try:
            ...
            # Obtém a pasta Itens Enviados (olFolderSentMail = 5)
            sent_folder = namespace.GetDefaultFolder(5)
            
            # Obtém todos os itens e ordena por data de envio (mais recente primeiro)
            items = sent_folder.Items
            items.Sort("[SentOn]", True)
            
            # Define o termo de busca baseado no tipo de template
            if template_type == "Sinistro":
                # Procura pelo último e-mail enviado para o endereço específico
                template_email = None
                for item in items:
                    try:
                        if "Analise Sinistro Auto" in item.To:
                            template_email = item
                            break
                    except:
                        continue
            else:
                # Mantém a lógica existente para outros tipos
                search_term = {
                    "Alerta 8": "Alerta 8",
                    "Alerta 9": "Alerta 9",
                    "PPE": "PPE"
            ...
            if template_email:
                # Cria um novo e-mail usando Forward para manter as imagens
                new_mail = template_email.Forward()
                new_mail.Subject = template_email.Subject
                new_mail.HTMLBody = template_email.HTMLBody
                
                # Limpa os campos de destinatário
                new_mail.To = ""
                new_mail.CC = ""
                ...
                QMessageBox.warning(
                    ...
                    "Não foi encontrado nenhum e-mail com os critérios especificados em Itens Enviados."
                )
            ...
            QMessageBox.warning(
                ...
                f"Erro ao buscar template: {str(e)}"
            )
    ...
    def show_most_used_phrases(self):
        """Mostra as frases mais utilizadas do botão ativo"""
        try:
            if not self.active_button:
                QMessageBox.warning(self, "Aviso", "Selecione um botão primeiro!")
                return
                
            # Carregar contagem de uso das frases
            try:
                with open('phrase_usage.json', 'r', encoding='utf-8') as f:
                    phrase_usage = json.load(f)
            except FileNotFoundError:
                phrase_usage = {}
            
            # Criar diálogo não-modal
            dialog = QDialog(self)
            dialog.setWindowTitle(f"Frases Mais Utilizadas - {self.active_button.text()}")
            dialog.setMinimumWidth(500)
            dialog.setMinimumHeight(400)
            dialog.setWindowFlags(dialog.windowFlags() | Qt.WindowStaysOnTopHint)  # Mantém janela sempre visível
            dialog.setModal(False)  # Define como não-modal
            
            layout = QVBoxLayout(dialog)
            
            # Lista de frases
            phrases_list = QListWidget()
            layout.addWidget(phrases_list)
            
            # Pegar frases do botão ativo
            button_phrases = self.frases.get(self.active_button.text(), [])
            phrases_with_usage = [(phrase, phrase_usage.get(phrase, 0)) for phrase in button_phrases]
            sorted_phrases = sorted(phrases_with_usage, key=lambda x: x[1], reverse=True)
            
            # Adicionar frases à lista
            for phrase, count in sorted_phrases:
                item = QListWidgetItem(f"{phrase} ({count} usos)")
                phrases_list.addItem(item)
            
            def copy_and_keep_open(item):
                """Copia a frase sem fechar a janela"""
                text = item.text()
                phrase = re.sub(r'\s*\(\d+\s*usos\)$', '', text)
                
                clipboard = QApplication.clipboard()
                clipboard.setText(phrase)
                
                phrase_usage[phrase] = phrase_usage.get(phrase, 0) + 1
                with open('phrase_usage.json', 'w', encoding='utf-8') as f:
                    json.dump(phrase_usage, f, ensure_ascii=False, indent=4)
                
                item.setText(f"{phrase} ({phrase_usage[phrase]} usos)")
                self.statusBar().showMessage(f"Frase copiada: {phrase}", 2000)
            
            phrases_list.itemClicked.connect(copy_and_keep_open)
            dialog.show()  # Usa show() em vez de exec()
            
        except Exception as e:
            QMessageBox.warning(self, "Erro", f"Erro ao mostrar frases: {str(e)}")
...
class PDFViewer(QWidget):
    def __init__(self, parent=None):
        ...
        # Configurar cor de fundo para o visualizador
        self.web_view.page().setBackgroundColor(QColor(47, 53, 59))
        
        # Injetar CSS para tema escuro
        dark_mode_js = """
        (function() {
            document.documentElement.style.colorScheme = 'dark';
            document.body.style.backgroundColor = '#2F353B';
        ...
        """
        self.web_view.page().runJavaScript(dark_mode_js)
