...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Widget de abas personalizadas para os sites
        ...
        # Aba fixa de relação de sites
        sites_list_tab = QWidget()
        sites_list_layout = QHBoxLayout()
        ...
        add_site_btn = QPushButton("Cadastrar Site")
        remove_site_btn = QPushButton("Remover Site")
        add_site_btn.setFixedWidth(150)
        remove_site_btn.setFixedWidth(150)
        add_site_btn.clicked.connect(self.add_site_dialog)
        remove_site_btn.clicked.connect(self.remove_site_dialog)
        
        left_layout.addWidget(add_site_btn)
        left_layout.addWidget(remove_site_btn)
        ...
        # Área de thumbnails (grid)
        scroll = QScrollArea()
        scroll_content = QWidget()
        self.sites_grid = QGridLayout(scroll_content)
        self.sites_grid.setSpacing(20)
        scroll.setWidget(scroll_content)
        scroll.setWidgetResizable(True)
        
        # Adicionar elementos ao layout da aba de relação
        sites_list_layout.addWidget(left_panel)
        sites_list_layout.addWidget(separator)
        sites_list_layout.addWidget(scroll)
        sites_list_tab.setLayout(sites_list_layout)
        
        # Adicionar aba fixa
        self.sites_tabs.addTab(sites_list_tab, "Relação de Sites")
        self.sites_tabs.tabBar().setTabButton(0, QTabBar.RightSide, None)  # Remove botão fechar
        
        tab_container_layout.addWidget(self.sites_tabs)
        tab_container.setLayout(tab_container_layout)
        quinta_layout.addWidget(tab_container)
        
        quinta_aba.setLayout(quinta_layout)
        self.tab_widget.addTab(quinta_aba, "Sites")
            ...
            try:
                ...
                for button_name in botoes_ordenados:
                    ...
                    for row in sheet.rows:
                        sheet.row_dimensions[row[0]].height = 15
            ...
            for botao, cb in checkboxes.items():
                if cb.isChecked():
                    ...
                    for frase in self.selected_texts:
                        if frase:
                            # Verifica se a frase já existe no bot��o
        ...
        if dialog.exec_() == QDialog.Accepted:
            try:
                ...
                if alteracoes:
                    ...
                    # Salvar no arquivo
                    if self.save_phrases_to_file():
                        # Atualizar os textos dos bot��es existentes
    ...
    def add_site_dialog(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Cadastrar Novo Site")
        dialog.setMinimumWidth(500)  # Aumentado a largura
        dialog.setMinimumHeight(250)  # Aumentado a altura
        layout = QFormLayout()
        
        # Campos do formulário com tamanhos aumentados
        ...
        username_input.setMinimumWidth(350)
        password_input.setMinimumWidth(350)
        
        # Configurar altura dos campos
        height = 30
        name_input.setFixedHeight(height)
        url_input.setFixedHeight(height)
        username_input.setFixedHeight(height)
        password_input.setFixedHeight(height)
        
        password_input.setEchoMode(QLineEdit.Password)
        
        # Placeholder texts
        name_input.setPlaceholderText("Digite o nome do site")
        url_input.setPlaceholderText("Digite a URL (www.exemplo.com ou http(s)://exemplo.com)")
        username_input.setPlaceholderText("Digite o usuário")
        password_input.setPlaceholderText("Digite a senha")
        
        # Adicionar campos ao layout com espaçamento
        layout.addRow("Nome do Site:", name_input)
        layout.addRow("URL:", url_input)
        layout.addRow("Usuário:", username_input)
        layout.addRow("Senha:", password_input)
        
        # Ajustar espaçamento entre os campos
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if not site_data['name'] or not site_data['url']:
                QMessageBox.warning(dialog, "Erro", "Nome do site e URL são obrigatórios!")
                return
    ...
    def add_site_thumbnail(self, site_data):
        ...
        try:
            ...
            site_frame = QFrame()
            site_frame.setFrameStyle(QFrame.Box | QFrame.Plain)
            ...
            site_name_label.setMinimumHeight(25)  # Altura mínima para a fonte
            site_name_label.setMaximumHeight(25)  # Altura máxima para manter proporção
            site_name_label.setAlignment(Qt.AlignCenter)
            ...
            """)
            preview_label.setText("Carregando preview...")
            preview_label.clicked.connect(lambda: self.open_site_in_tab(site_data))
            
            # Frame de login
            login_frame = QFrame()
            login_frame.setFrameStyle(QFrame.Box | QFrame.Plain)
            ...
            self.capture_site_preview(site_data['url'], preview_label)
    ...
    def capture_site_preview(self, url, preview_label):
        ...
        # Configurar página com políticas de segurança adequadas
        page = view.page()
        profile = page.profile()
        
        # Configurar settings para preview
        ...
        def handle_load_finished(success):
            if success:
                QTimer.singleShot(5000, take_snapshot)
            else:
                preview_label.setText("Erro ao carregar site")
                view.deleteLater()
        ...
        def take_snapshot():
            try:
                ...
                if scaled_pixmap.width() > 250 or scaled_pixmap.height() > 180:
                    scaled_pixmap = scaled_pixmap.copy(
                        (scaled_pixmap.width() - 250) // 2,
                ...
                preview_label.setText("Erro ao capturar preview")
    ...
    def close_site_tab(self, index):
        ...
        if index > 0:  # Não fecha a aba de relação
